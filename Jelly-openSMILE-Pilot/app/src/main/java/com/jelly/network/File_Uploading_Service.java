...
/**
 * Created by tiantianfeng on 10/22/17.
 */

public class File_Uploading_Service extends Service {
    ...
    * */
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if(startId > 0) {
            ...
            Log.d(Constants.DEBUG, "File_Uploading_Service->onStartCommand->stopSelfResult");
        }
    ...
    private Runnable mTickExecutor = new Runnable() {
        ...
        public void run() {
            ...
            Intent battery_Intent = new Intent(getApplicationContext(), File_Uploading_Service.class);
    ...
    private Runnable fileTickExecutor = new Runnable() {
        ...
        public void run() {
            ...
            if(isDeviceOnline() && retrieveSharedPreference(Constants.UPLOAD_STATUS).contains(Constants.UPLOAD_ON)) {
                ...
                if(currentFileIndex >= files.length) {
                    ...
                    if(files[currentFileIndex].getName().contains("csv")) {
                        ...
                        seconds = seconds / 512;
                        Log.d(Constants.DEBUG, "File_Uploading_Service->File Size(KB)->" + files[currentFileIndex].length()/1024);
                        Log.d(Constants.DEBUG, "File_Uploading_Service->Seconds->" + seconds);
    ...
    private void startUploadingService() {
        ...
        */
        ...
        Intent upload_Service = new Intent(this, File_Uploading_Service.class);
            ...
            if(newState.toString().contains("COMPLETED")) {
                ...
                for( Map.Entry me : map.entrySet() )
                {
                    ...
                    if (Integer.parseInt(me.getKey().toString()) == id) {
                        ...
                        if(file.exists()) {
                            Log.d(DEBUG, "Transmit complete, Delete file");
                            Log.d(Constants.DEBUG, "File_Uploading_Service->UploadListener->onStateChanged->" + file.getName());
                            file.delete();
                        }
                ...
                if(csvFile.exists()) {
                    if (csvFile.listFiles().length < 1) {
                        Log.d(Constants.DEBUG, "File_Uploading_Service->UploadListener->Start New transmit");
